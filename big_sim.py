...
from code1.foundations.helpers import scale_input_theory
from brian2 import clear_cache, uA, mV, ms
from code1.models.models import Barrel_PC, Barrel_IN
from code1.foundations.helpers import scale_to_freq
from code1.foundations.make_dynamic_experiments import make_dynamic_experiments
import numpy as np
import pandas as pd
...
for _ in range(N_runs):
    ...
    [input_theory, dynamic_theory, hidden_state, weights_PC, theta_PC] = make_dynamic_experiments(qon_qoff_type, baseline, tau_PC, factor_ron_roff, mean_firing_rate_PC, sampling_rate, duration_PC)
    ...
    results_PC = pd.concat([results_PC, data], ignore_index=True)
...
for _ in range(N_runs):
    ...
    [input_theory, dynamic_theory, hidden_state, weights_IN, theta_IN] = make_dynamic_experiments(qon_qoff_type, baseline, tau_IN, factor_ron_roff, mean_firing_rate_IN, sampling_rate, duration_IN)
    ...
    data = pd.DataFrame(data=data, index=vars_to_track).T
    results_IN = pd.concat([results_IN, data], ignore_index=True)
    

# Save data
#results_PC.to_pickle('results/results_PC_spiket.pkl')
#results_IN.to_pickle('results/results_IN_spiket.pkl')
